...
curl -X PUT "http://${user}:${pass}@localhost:5984/_node/couchdb@${node_slave_two}/_config/chttpd/bind_address" -d '"0.0.0.0"'
curl -X PUT "http://${user}:${pass}@localhost:5984/_node/couchdb@${node_slave_two}/_config/httpd/bind_address" -d '"0.0.0.0"'
